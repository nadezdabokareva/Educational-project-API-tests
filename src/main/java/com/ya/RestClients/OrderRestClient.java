...
public class OrderRestClient extends BasicRestClient {
    ...
    public static ValidatableResponse closeOrder(Order order) {
        return given()
                .spec(getBaseSpec())
                .body(order)
                .params(new HashMap<>(){{put("id",order.getId());}})
                .when()
                .put(ORDER_PATH + "finish/" + order.getId())
                .then().log().all();
    }
    ...
    public static ValidatableResponse getListOrders(Integer courierId, String nearestStation,
